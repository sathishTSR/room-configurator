...
function DroppedFurniture({ position }: { position: [number, number, number] }) {
  return (
...
function PreviewBox({ droppingAsset, addDroppedAsset, setDroppingAsset }: { droppingAsset: { position: [number, number, number]; type: string } | null; addDroppedAsset: (asset: { type: string; position: [number, number, number] }) => void; setDroppingAsset: (asset: { position: [number, number, number]; type: string } | null) => void; }) {
...
export default function Builder() {
  const [droppingAsset, setDroppingAsset] = useState<{ position: [number, number, number]; type: string } | null>(null);
      ...
      >
        <Canvas camera={{ position: [0, 3, 5] }}>
          <CanvasWithRefs>
            ...
            <Wall />
            <DroppedAssets />
            <PreviewBox droppingAsset={droppingAsset} addDroppedAsset={addDroppedAsset} setDroppingAsset={setDroppingAsset} />
            <OrbitControls />
          </CanvasWithRefs>
        </Canvas>
      </div>
